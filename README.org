...
Contents of this file get tangled to $HOME directory via
...
#+RESULTS:
: ("~/.config/nixpkgs/overlays/emacs/emacs.desktop" "~/.config/nixpkgs/overlays/emacs/init.el" "~/.config/nixpkgs/overlays/01-emacs.nix" "~/.config/nixpkgs/overlays/99-tom.nix" "~/.config/nixpkgs/config.nix")
...
#+RESULTS:
#+begin_example
replacing old 'conda-shell'
replacing old 'emacs-with-packages-26.3'
replacing old 'emacs-custom-desktop'
replacing old 'gmsh-4.4.1'
replacing old 'gnumake-4.2.1'
replacing old 'gnupg-2.2.19'
replacing old 'htop-2.2.0'
replacing old 'nix-2.3.1'
replacing old 'password-store-1.7.3'
replacing old 'ripgrep-11.0.2'
replacing old 'slack-4.2.0'
replacing old 'suitesparse-5.4.0'
replacing old 'xclip-0.13'
installing 'conda-shell'
installing 'emacs-with-packages-26.3'
installing 'emacs-custom-desktop'
installing 'gmsh-4.4.1'
installing 'gnumake-4.2.1'
installing 'gnupg-2.2.19'
installing 'htop-2.2.0'
installing 'nix-2.3.1'
installing 'password-store-1.7.3'
installing 'ripgrep-11.0.2'
installing 'slack-4.2.0'
installing 'suitesparse-5.4.0'
installing 'xclip-0.13'
#+end_example


* Setting up a new computer

This is a checklist for myself.

1. Install Ubuntu. Remember to encrypt hard drives.
2. Install tools outside of Nix: dropbox, docker, vim, git, nix.
3. Symlink $HOME/.gnupg.
...
enabled via the following config.nix:
...
The package Emacs gets customized in the following overlay which defines also a
derivation for the custom desktop-file (given below):
...
The custom desktop-file unsets XMODIFIERS due to an issue with deadkeys:
...
Contains some additional environment variables.

#+begin_src :tangle ~/.profile
...
export PASSWORD_STORE_DIR=$HOME/Dropbox/Config/.password-store
#+end_src
