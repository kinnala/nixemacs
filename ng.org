...
of straight.el which manages Emacs Lisp packages.

* Derivation for straight.el

The initial version of straight.el is patched slightly so that it doesn't
perform any HTTP requests while bootstrapping.  All calls to
githubusercontent.com are replaced by file reads from Nix store.
...
#+begin_src elisp :mkdirp yes :tangle ~/tmp/.emacs.d/straight/versions/default.el
...
#+begin_src elisp :mkdirp yes :tangle ~/tmp/.emacs.d/init.el
...
(define-key global-map (kbd "<f10>")
  (lambda () (interactive)
    (find-file "~/src/nixos/readme.org")))
